...
public class SWGenLakes extends WorldGenerator
{
    private final Block block;

    public SWGenLakes(Block blockIn)
    {
        this.block = blockIn;
    }
                    ...
                    for (int j4 = 4; j4 < 8; ++j4)
                    {
                        if (aboolean[(i2 * 16 + j3) * 8 + j4])
                        {
                            ...
                            if (worldIn.getBlockState(blockpos).getBlock() == Blocks.DIRT && worldIn.getLightFor(EnumSkyBlock.SKY, position.add(i2, j4, j3)) > 0)
                            {
                                ...
                                if (biome.topBlock.getBlock() == ModBlocks.MYCELIUM_REFRACTED_BLOCK)
                                {
                                    worldIn.setBlockState(blockpos, ModBlocks.MYCELIUM_REFRACTED_BLOCK.getDefaultState(), 2);
                                }
                                else
                                {
                ...
                for (int j2 = 0; j2 < 16; ++j2)
                {
                    for (int k3 = 0; k3 < 16; ++k3)
                    {
                        for (int k4 = 0; k4 < 8; ++k4)
                        {
                            ...
                            {
                                worldIn.setBlockState(position.add(j2, k4, k3), ModBlocks.REFRACTEDSTONE_BLOCK.getDefaultState(), 2);
                            }
                        }
                    }
                }
