...
public class GenLayerSWKeyBiomes extends GenLayer {

	public GenLayerSWKeyBiomes(long l, GenLayer genlayer) {
		super(l);
		parent = genlayer;
	}

	public GenLayerSWKeyBiomes(long l) {
		super(l);
	}
	...
	private int getKeyBiomeFor(int mapX, int mapZ, int index) {
		...
		switch ((index + offset) % 4) {
			case 0:
			default:
				return Biome.getIdForBiome(BiomeInit.REFRACTED_FOREST);
			case 1:
				return Biome.getIdForBiome(BiomeInit.REFRACTED_FOREST);
			case 2:
				return Biome.getIdForBiome(BiomeInit.RF_SHROOM);
			case 3:
				return Biome.getIdForBiome(BiomeInit.R_ROOFED_FOREST);
		}
	}
}
