...
public class EntityWebProjectileEgg extends EntityThrowable{

	public EntityWebProjectileEgg(World par1World) {
		super(par1World);
	}

	public EntityWebProjectileEgg(World par1World, EntityLivingBase par2EntityLiving) {
	...
	private void makeTrail() {
		for (int i = 0; i < 2; i++) {
			...
			world.spawnParticle(EnumParticleTypes.TOWN_AURA, dx, dy, dz, 0.0D, 0.0D, 0.0D);
		}
	}
	...
	private void die() {
		if (!this.world.isRemote) {
			this.playSound(SoundEvents.ENTITY_LLAMA_SPIT, 1.0F, 1.0F / (rand.nextFloat() * 0.9F + 0.8F));
			this.setDead();
			...
			int k = (int) this.posZ;
			Entity entity = this;
			world.setBlockState(new BlockPos(i, j , k), ModBlocks.SPIDERSACK_BLOCK.getDefaultState(), 3);
		}
		}
}
